...
async function _handleCECommand(text) {
    ...
    if (t === "/ce" || t === "ce") {
        ...
        await writeMode(!cur);
        return !cur ? "✅ CE mode ON — I'll translate every message and voice note." : "⏹ CE mode OFF — normal conversation resumed.";
    }
...
export async function processOneMessage(full, deps) {
    ...
    // --- CE command: handle /ce, /ce on, /ce off directly (no agent) ---
    const ceReply = await _handleCECommand(textBody);
    if (ceReply !== null) {
        const ceTo = full.from_user_id ?? "";
        const ceCtx = full.context_token ?? undefined;
        try {
            await sendMessageWeixin({ to: ceTo, text: ceReply, opts: { baseUrl: deps.baseUrl, token: deps.token, contextToken: ceCtx } });
    ...
    const mediaOpts = {};
    // CE mode flag — read before media download so a concurrent /ce off is already on disk
    ...
    if (_ceActive && (_hasVoice || _hasText)) {
        ...
        try {
            ...
            if (_hasVoice) {
                ...
                if (voiceText) { pythonArgs = ["--text", voiceText]; ceTimeout = 60000; }
                else if (mediaOpts.decryptedVoicePath) { pythonArgs = ["--file", mediaOpts.decryptedVoicePath]; ceTimeout = 180000; }
                else { pythonArgs = []; ceTimeout = 0; }
            } else {
                pythonArgs = ["--text", textBody];
                ceTimeout = 60000;
            }
            if (pythonArgs.length > 0) {
                const { stdout: pyOut } = await new Promise((resolve, reject) => {
                ...
                if (raw) {
                    ...
                    if (ceAudioPath) {
                        // Send MP3 voice file + translation text together
                        await sendWeixinMediaFile({
                            filePath: ceAudioPath,
                            to: ceTo,
                            text: ceTranslation,
                            opts: { baseUrl: deps.baseUrl, token: deps.token, contextToken: ceContextToken },
                            cdnBaseUrl: deps.cdnBaseUrl,
                        });
                        logger.info(`[ce] intercept: sent audio+text to=${ceTo} audio=${ceAudioPath}`);
                    } else if (ceTranslation) {
                        // Text-only fallback (no audio generated)
                        await sendMessageWeixin({ to: ceTo, text: ceTranslation, opts: { baseUrl: deps.baseUrl, token: deps.token, contextToken: ceContextToken } });
                        logger.info(`[ce] intercept: sent text-only translation to=${ceTo}`);
                    }
                }
            }
        } catch (ceErr) {
            deps.errLog(`[ce] intercept error: ${String(ceErr)}`);
        }
        return;
    }
